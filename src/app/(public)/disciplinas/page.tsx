		...
		<main>
			...
			<section>
				<div className={"MaxWidthWrapper"}>
					...
					>
						<Hint
							desktop="Clique em uma disciplina para ter acesso a sua ementa."
							mobile="Aperte em uma disciplina para ter acesso a sua ementa."
						/>
					</div>
					<div className={styles.list} id={styles.listaSemestres}>
						{semestres.map((semestre, index) => (
							<>
								...
								<div
									key={index}
									className={[styles.card, "scrollAnimation"].join(" ")}
									style={{ transitionDelay: `${index * 100}ms` }}
								>
									<h2>{semestre.title}</h2>
									<ul>
										{semestre.lista.map((disciplina, index) => (
											<a target="_blank" href={disciplina.link} key={index}>
												<span>{disciplina.name}</span>
												{disciplina.link && <MdOutlineFileDownload />}
											</a>
										))}
									</ul>
								</div>
